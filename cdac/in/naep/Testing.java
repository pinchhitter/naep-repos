...
class Data{
	...
	void printHeader(){
		...
		for(String key: questions.keySet() ){
			...
			if( question.timeTakenPercentiles.size() > 2){
		...
		System.out.println("@attribute applicant string");	
	...
	void printData(Data master){
		...
		for( String id: students.keySet() ){
			...
			if( totalTimePercentile.get( student.totalTime ) != null){
			...
			for(String key: master.questions.keySet() ){
				...
				if( master.questions.get( key ).timeTakenPercentiles.size() > 2){
...
class Testing{
	...
	void createDataSet(Data data){
		...
		for(String id: students.keySet() ){
			...
			Student studentN = new Student( id );
			...
			for(String activity: student.activities ){
				...
				if( startTime == null ){
					...
					try{
						...
						studentN.activities.add( activity );
						studentN.totalTime = timeTaken;
			...
			data.students.put( id, studentN );
		}
		System.err.println(data.students.size() );
	}
			

	void read( String studentFile, boolean headerStudent ){
	...
	void createDataSet( ){
		...
		this.students = new TreeMap<String, Student>();
		this.questions = new TreeMap<String, Question>();
		read( "../data/data_a_hidden_30.csv", true);
		createDataSet( data30 );

		this.students = new TreeMap<String, Student>();
		this.questions = new TreeMap<String, Question>();
		read( "../data/data_a_hidden_20.csv", true );
		createDataSet( data20 );

		this.students = new TreeMap<String, Student>();
		this.questions = new TreeMap<String, Question>();
		read( "../data/data_a_hidden_10.csv", true);
		createDataSet( data10 );
